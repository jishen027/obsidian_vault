# 虚拟私有云 - VPC

> **Amazon VPC (Virtual Private Cloud)** 让您能在 AWS 云中启动资源在您逻辑定义的虚拟网络中。它是 AWS 网络架构的核心服务，提供网络隔离、安全控制和灵活的网络配置。
>
> 相关文档：[[Security Group]] | [[NACL]] | [[S3]] | [[DynamoDB]] | [[VPC Peering]] | [[Virtual Private Gateway]] | [[Customer Gateway]] | [[ENI]] | [[Subnet]] | [[Route Table]] | [[NAT Gateway]] | [[Internet Gateway]] | [[NAT Instance]] | [[CIDR]] | [[VPC Endpoints]] | [[VPC Flow Logs]] | [[VPC Traffic Mirroring]] | [[Site to Site VPN]] | [[Transit Gateway]] | [[Direct Connect]] | [[AWS Outposts]]

---

## 1. 基础架构与寻址

### VPC 核心特性

| 特性 | 说明 |
|------|------|
| **区域性** | VPC 是区域级服务，无法跨区域 |
| **[[CIDR]] 范围** | `/16` 到 `/28`（65,536 到 16 个 IP） |
| **主 CIDR** | 创建时指定，不可修改，且**永久不可删除**（除非删除整个 VPC） |
| **次级 CIDR** | 可随时追加为 VPC 扩容地址空间，**在其中没有资源占用地址时可以移除**；完整规则和重叠限制见 [[CIDR]] 独立笔记 |

### 子网 IP 预留规则

> **AWS 在每个子网中预留 5 个 IP 地址**，规划容量时必须考虑此损耗。

| IP 地址 | 用途 |
|---------|------|
| `x.x.x.0` | 网络地址 |
| `x.x.x.1` | 隐式路由器 |
| `x.x.x.2` | DNS 解析 |
| `x.x.x.3` | 未来用途 |
| `x.x.x.4` | 未来用途（NAT 需要） |
| `x.x.x.255` | 广播地址（保留） |

### 子网类型

| 类型 | 特点 | 路由表 |
|------|------|--------|
| **公有子网** | 有到 [[Internet Gateway|IGW]] 的路由 | `0.0.0.0/0 → IGW` |
| **私有子网** | 无直接 IGW 访问 | 通过 [[NAT Gateway|NAT 网关]]出站 |

> [[Internet Gateway]] 的完整特性、公有子网"四件套"判定条件及与 NAT 网关/[[Virtual Private Gateway|VGW]] 的对比见 [[Internet Gateway]] 独立笔记

> 子网的公私判定逻辑、路由表关联规则、IPv6 子网、与 NACL/Security Group 的关系等完整内容见 [[Subnet]] 独立笔记

---

## 2. 路由与互联网接入

### 路由表

- 每个子网必须关联一个路由表
- 默认路由表自动创建，可修改
- 路由表可以关联多个子网，但一个子网只能关联一个路由表

> 路由表的完整目标类型、最长前缀匹配、路由传播机制等见 [[Route Table]] 独立笔记

### 出站技术对比（考试重点）

| 特性 | [[NAT Gateway|NAT 网关]] | [[NAT Instance|NAT 实例]] |
|------|---------|---------|
| **管理方式** | 完全托管 | 自行管理 EC2 |
| **带宽扩展** | 自动（100 Gbps） | 手动（取决于实例类型） |
| **可用性** | 高可用（AZ 内） | 需自行配置 |
| **源/目的地检查** | 无需配置 | **必须禁用** |
| **成本** | 按使用量计费 | EC2 + EIP 费用 |
| **位置要求** | 必须在公有子网 | 可在任意子网 |

> 完整对比（安全组支持、端口转发、堡垒机复用等）及高可用部署方式见 [[NAT Gateway]] 独立笔记

### 网络组件关系

```
私有子网实例 → [[NAT Gateway|NAT 网关]] (公有子网) → 弹性 IP (EIP) → IGW → 互联网
```

---

## 3. 安全防御层

### 安全组 vs NACL 对比（考试高频考点）

| 特性 | 安全组 ([[Security Group]]) | NACL ([[NACL]]) |
|------|--------------------------|----------------|
| **作用层级** | 实例级 | 子网级 |
| **状态** | **有状态** | **无状态** |
| **规则类型** | 仅允许规则 | 允许 + 拒绝规则 |
| **默认规则** | 拒绝所有入站，允许所有出站 | 允许所有入站和出站 |
| **自定义默认** | - | 拒绝所有 |
| **评估顺序** | 所有规则 | 按规则编号从小到大 |
| **配置级别** | 需要知道实例 ID | 只需知道子网 |

### 有状态 vs 无状态（考试重点）

| 类型 | 说明 | 示例 |
|------|------|------|
| **有状态（安全组）** | 允许的请求响应自动放行 | 出站允许 HTTPS，入站响应自动允许 |
| **无状态（NACL）** | 入站和出站规则独立 | 入站允许 80 端口，出站也需单独允许响应流量 |

### 默认安全组行为

| 组件 | 默认安全组规则 |
|------|--------------|
| **入站** | 拒绝所有 |
| **出站** | 允许所有 |
| **自定义安全组** | 创建时为空，需手动添加规则 |

### WAF 与 Shield

| 服务 | 防护层级 | 防护类型 |
|------|---------|---------|
| **[[AWS WAF]]** | 第 7 层 | SQL 注入、XSS、HTTP/HTTPS 攻击 |
| **[[AWS Shield]]** | 第 3/4 层 | DDoS 攻击（Standard 免费，Advanced 付费） |

---

## 4. VPC 终端节点与 PrivateLink

- **网关终端节点（Gateway）**：仅支持 S3/DynamoDB，基于路由表条目，完全免费
- **接口终端节点（Interface）**：基于 **PrivateLink** 技术，覆盖大多数服务，创建 [[ENI]]，按小时+数据处理量计费
- **PrivateLink（VPC Endpoint Service）**：可反过来将自有 NLB 服务私密共享给其他账号，是"仅暴露单一服务"而非"打通整个网络"的跨账号共享方案

> 完整的两种终端节点对比、私有 DNS 配置、终端节点策略、PrivateLink 架构及与 API Gateway/VPC Peering 的对比见 [[VPC Endpoints]] 独立笔记

---

## 5. 高级连接

### VPC 对等连接

| 特性 | 说明 |
|------|------|
| **范围** | 同区域或跨区域 VPC 连接 |
| **路由** | 需在双方 VPC 添加路由 |
| **单播** | 不支持多播/广播 |
| **成本** | 免费（但跨区域有数据传输费） |
| **限制** | 不支持重叠 CIDR，**不支持传递路由** |

> 完整的限制、DNS 解析配置、安全组互相引用规则及与 Transit Gateway 的对比，见 [[VPC Peering]] 独立笔记

### VPC 共享（VPC Sharing，考试易混淆点）

> **VPC 共享**是 AWS Resource Access Manager (RAM) 的一项功能，允许 VPC 拥有者（owner）把**一个或多个子网**共享给同一 AWS Organizations 下的其他账户（participant）。参与方账户可以直接在共享子网里创建 [[EC2]]、[[RDS]] 等资源，就像这些子网属于自己的 VPC 一样——本质是**多个账户共用同一个 VPC**，而不是两个独立 VPC 之间建立连接。

| 维度 | VPC 共享（VPC Sharing） | VPC 对等连接（VPC Peering） |
|------|--------------------------|------------------------------|
| **本质** | **多个账户共用同一个 VPC**（共享子网） | **两个独立 VPC** 之间建立连接 |
| **能否共享 VPC 本身** | 只能共享**子网**，不能共享整个 VPC | 不适用——Peering 连接的双方本来就是两个独立 VPC，不存在"共享" |
| **是否central化管理** | ✅ 是，VPC 拥有者集中管理网络（路由表、NACL、安全组等），参与方只管理自己创建的资源 | ❌ 否，两个 VPC 各自独立管理，只是打通了网络连接 |
| **参与方能否看到彼此资源** | 不能——参与方之间即使共用同一 VPC 子网，也**看不到、改不了、删不了**其他参与方或 owner 的资源 | 不适用 |
| **典型场景** | 需要给 Organizations 下多个部门/账户提供**统一、集中管理**的网络环境，同时保留各自的计费和访问控制边界 | 两三个独立 VPC 之间的简单直连 |

> **考试陷阱：题目描述"希望在同一 AWS Organizations 下的多个账户之间提供共享的、集中管理的 VPC，用于高互联互通性的应用"时，正确答案是 VPC 共享（共享子网），不是 VPC Peering**——VPC Peering 只是连接两个独立的 VPC，既不能"共享 VPC 本身"，也不提供中心化管理能力；而且无论共享还是 Peering，**账户所有者都不能把整个 VPC 作为一个整体共享出去**，VPC 共享共享的对象始终是**子网**这一层级。

### 站点到站点 VPN

| 组件 | 说明 |
|------|------|
| **[[Virtual Private Gateway\|VGW]]** | AWS 侧的 VPN 集中器 |
| **[[Customer Gateway\|CGW]]** | 用户本地数据中心的物理设备或软件 |
| **类型** | 基于路由 (Route-based) 或基于目标 (Target-based) |

> 隧道冗余、静态/动态（BGP）路由、ECMP 聚合带宽、Accelerated VPN、VPN CloudHub 及与 Direct Connect 的对比见 [[Site to Site VPN]] 独立笔记

### Transit Gateway

| 特性 | 说明 |
|------|------|
| **架构** | 中心枢纽（星型拓扑） |
| **规模** | 支持成百上千个 VPC 连接 |
| **路由** | 支持传递性路由 |
| **适用场景** | 大规模多 VPC 架构 |

> 附件类型、专属路由表与网络分段、跨账号共享（AWS RAM）、跨区域 Peering 等完整内容见 [[Transit Gateway]] 独立笔记

### [[Direct Connect]] (DX)

| 特性 | 说明 |
|------|------|
| **类型** | 专用物理连接 |
| **延迟** | 亚毫秒级 |
| **安全性** | 不走公网 |
| **加密** | 默认不加密，需结合 VPN |
| **适用场景** | 大规模数据传输、低延迟需求 |

> 专用/托管连接类型、VIF、Direct Connect Gateway、LAG、高可用弹性模型等完整内容见 [[Direct Connect]] 独立笔记

### [[AWS Outposts]]

| 特性 | 说明 |
|------|------|
| **本质** | AWS 硬件物理部署在客户自有机房，其上的子网是**现有 VPC 的自然延伸** |
| **本地网络访问** | 通过**本地网关（Local Gateway）**访问本地站点网络 |
| **与父区域连接** | 依赖 Direct Connect/Site-to-Site VPN，用于控制平面操作 |
| **适用场景** | 数据主权/合规要求数据留在本地，或需要对本地系统超低延迟 |

> 与 Local Zones/Wavelength 的对比、支持的服务范围等完整内容见 [[AWS Outposts]] 独立笔记

---

## 6. 限制和配额

| 限制项 | 默认值 |
|--------|--------|
| **每个区域的 VPC 数量** | 1（可申请提升到 100） |
| **每个 VPC 的子网数量** | 无硬限制 |
| **每个子网的实例数量** | 取决于可用 IP 数量 |
| **每个[[Route Table\|路由表]]的路由数** | 最多 50 条 |

---

## 7. 定价模型

### 计费项

| 计费项 | 说明 |
|--------|------|
| **VPC** | **免费** |
| **公有子网** | **免费** |
| **[[NAT Gateway|NAT 网关]]** | 按运行时间 + 数据处理量 |
| **[[NAT Instance|NAT 实例]]** | EC2 + EIP 费用 |
| **VPC 对等连接** | 跨区域数据传输费 |
| **[[Transit Gateway]]** | 按连接数 + 数据处理量 |
| **[[Direct Connect]]** | 端口带宽费用 |
| **[[VPC Endpoints\|网关终端节点]]** | **免费** |
| **[[VPC Endpoints\|接口终端节点]]** | 按小时 + 数据处理量 |
| **PrivateLink** | 按小时 + 数据处理量 |

> **重要**：VPC 本身和网关终端节点是免费的！

---

## 8. 考试重点总结

### SAA-C02 高频考点

1. **VPC 区域性**：VPC 是区域级服务，无法跨区域
2. **子网预留 IP**：每个子网预留 5 个 IP
3. **[[NAT Gateway|NAT 网关]] vs 实例**：[[NAT Gateway|NAT 网关]]自动扩展，需禁用源/目的地检查
4. **安全组 vs NACL**：有状态 vs 无状态，允许 vs 允许+拒绝
5. **[[VPC Endpoints|VPC 终端节点]]**：接口（大多数服务）vs 网关（仅 S3/DynamoDB，免费）
6. **PrivateLink**：跨账号服务共享，NLB + 服务提供者模型，完整对比见 [[VPC Endpoints]] 独立笔记
7. **[[Transit Gateway]]**：大规模多 VPC 架构
8. **[[Direct Connect]]**：专用物理连接，不走公网
9. **[[VPC Peering]]**：不支持重叠 CIDR，不支持传递路由
10. **VPC 免费**：VPC 本身和网关终端节点免费

### 场景题解题思路

```
场景分析 → 选择网络方案
├── "跨账号 VPC 服务共享" → PrivateLink（详见 [[VPC Endpoints]]）
├── "私密访问 S3/DynamoDB" → 网关终端节点
├── "私密访问其他 AWS 服务" → 接口终端节点
├── "大规模多 VPC 架构" → [[Transit Gateway]]
├── "本地数据中心连接" → [[Direct Connect]] / VPN
├── "两个 VPC 互访" → [[VPC Peering]]
├── "私有子网出站" → [[NAT Gateway|NAT 网关]]
└── "DDoS 防护" → [[AWS Shield]] + [[AWS WAF]]
```

### 常见考试场景分析

**场景：需要在不同 AWS 账号之间安全地共享 VPC 内运行的 NLB 应用？**

| 选项 | 是否正确 | 原因 |
|------|---------|------|
| **PrivateLink** | ✅ 正确 | NLB + 跨账号 + 服务提供者模型 |
| [[VPC Peering]] | ❌ 错误 | 需要复杂路由，不适合服务提供者模型 |
| 互联网 | ❌ 错误 | 不安全，流量经过公网 |
| API Gateway | ❌ 错误 | 主要用于 RESTful API，不是 VPC 内服务共享 |

**关键总结：**
- **"跨账号 VPC 服务共享"** → PrivateLink
- **"私密访问 S3"** → 网关终端节点
- **"两个 VPC 互访"** → [[VPC Peering]]
- **"大规模多 VPC"** → [[Transit Gateway]]
- **"不走公网的本地连接"** → [[Direct Connect]]