...
### 请求节流（Throttling，考试易漏点）

- API Gateway 使用**令牌桶算法（Token Bucket Algorithm）**对请求做节流，防止后端被突发流量压垮：**稳定状态速率（Steady-State Rate）**限制平均每秒请求数，**突发容量（Burst）**对应令牌桶的最大容量，允许短时间内的流量尖峰
- 可在**账户级别**（所有 API 的默认上限）和**方法级别**（针对具体 API/阶段/方法的自定义限制）分别配置节流阈值
- 超出节流阈值的请求会收到 **429 Too Many Requests** 错误

> **考试要点：题目描述"需要缓冲/节流应对流量突增"的架构组合题时，API Gateway（节流）+ SQS/Kinesis（缓冲）是标准搭配**——API Gateway 本身具备节流能力，SQS 和 Kinesis 具备削峰填谷的缓冲能力，三者常一起作为正确答案组合出现；容易被当作干扰项的是 **ELB**（不具备节流能力）和 **Lambda 自身的并发限制**（超限时是直接以 429 报错拒绝请求，而不是主动做缓冲/节流的架构组件）。
