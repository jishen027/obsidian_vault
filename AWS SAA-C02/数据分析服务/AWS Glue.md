...
- **Glue Studio**：可视化界面，通过拖拽方式构建 ETL 作业流程，**底层仍然生成并运行 Apache Spark 脚本**，面向开发者/数据工程师，本质是"降低写 Spark 代码的门槛"而不是真正的零代码工具
- **Job Bookmark（作业书签）**：自动跟踪已处理过的数据，重复运行同一作业时只处理新增/未处理的数据，避免重复处理

### AWS Glue DataBrew（真正的零代码数据准备工具，考试易混淆点）

> **Glue DataBrew** 和 **Glue Studio** 名字很像、都提供可视化界面，但定位完全不同——DataBrew 面向**业务分析师/非技术用户**，用"配方（Recipe）"记录一系列清洗/转换步骤，**完全不需要接触任何代码或 Spark 脚本**；Glue Studio 生成的仍然是 Spark 脚本，本质上是给开发者用的低代码工具。

| 维度 | Glue DataBrew | Glue Studio | Amazon Athena |
|------|---------------|-------------|----------------|
| **目标用户** | 业务分析师、非技术数据准备人员 | 开发者/数据工程师 | 会写 SQL 的技术用户 |
| **是否涉及代码/脚本** | ❌ 完全不涉及，纯拖拽点选 | 底层仍是 **Spark 脚本**，只是可视化生成 | 需要写 **SQL** |
| **转换逻辑如何复用/共享** | **Recipe（配方）**：可保存、版本化、跨项目分享的转换步骤集合 | 分享 Job 定义（技术门槛更高） | 分享保存的 SQL 查询 |
| **数据剖析（Profiling）能力** | ✅ **内置**——自动生成列级统计（分布、基数、空值数、异常值） | ❌ 不内置，需要额外开发 | ❌ 不内置 |
| **数据血缘/审计** | Recipe 记录完整的转换步骤，天然可审计、可追溯 | 需要额外查看 Spark 脚本历史 | 需要额外查询历史记录 |

> **考试关键词识别**：题目强调**"零代码/无需编写或维护代码"** + **"业务分析师和数据工程师协作"** + **"需要数据剖析（profiling）和血缘追踪"** + **"能在团队间共享转换逻辑"**这组组合特征时，标准答案是 **Glue DataBrew**——不要被 Glue Studio 的"可视化画布"字面描述误导，Glue Studio 依然是面向开发者、生成 Spark 代码的工具，并且**不内置数据剖析能力**，这是它和 DataBrew 最本质的区别。
