...
- 使用 [[EC2#定价模式|EC2]] Spot 实例运行训练任务，可大幅降低训练成本（相比按需实例节省最高约 90%），SageMaker 自动处理 Spot 中断后的检查点保存和恢复，无需用户手动处理容错逻辑
...
### "实时推理 vs 批量推理"的通用二分法（考试重点：与上表的"四种方式"不是同一层概念）

> 有些题目不细分上表的四种具体部署方式，而是用更笼统的**"实时推理（Real-time Inference）vs 批量推理（Batch Inference）"二分法**来考查两者的**执行模式（同步/异步）**——这里的"批量推理"是**泛指**"离线、批量处理"这一类（对应上表的批量转换），"异步"在这里是**执行范式的统称**，不要与上表中 SageMaker 一个具体产品功能"**Asynchronous Inference（异步推理）**"混为一谈（后者是四种部署方式之一，前者是这道题里"批量推理"的一个笼统属性）。

| 维度 | 实时推理 (Real-time Inference) | 批量推理 (Batch Inference) |
|------|-------------------------------|---------------------------|
| **执行模式** | **同步（Synchronous）**——客户端发请求后**阻塞等待**，结果立即返回 | **异步（Asynchronous）**——客户端发请求后**不等待**，可以去做别的事，系统按自己的节奏处理并在之后返回结果 |
| **延迟** | 低延迟，适合交互式场景 | 延迟较高，但吞吐量更大 |
| **典型场景** | 推荐系统、聊天机器人、实时欺诈检测、自动驾驶 | 数据仓库/ETL 管道、周期性报表、预测性维护 |
| **调用方式（考试提示）** | REST API、gRPC、WebSocket、Serverless Functions | 批处理框架（Spark/Flink/Hadoop）、云批处理服务（[[AWS Batch]]）、按计划触发的脚本任务 |

> **考试陷阱**：选项常把"同步/异步"的对应关系**整体调换**（"批量推理是同步、实时推理是异步"），或把"调用方式"整体调换（"批量推理是 API 调用、实时推理是按计划调用"）——牢记：**实时 = 同步 = API 直接调用**；**批量 = 异步 = 排期/批处理框架触发**，这组映射的方向不能反。
...
> **考试陷阱：AWS 官方"ML Governance 三件套"是 Role Manager + Model Cards + Model Dashboard，不包含 Model Monitor 或 Clarify**——题目问"为了满足合规要求、保证模型透明可负责，SageMaker 提供了哪些**治理**工具"时，标准答案组合是这三个：**Role Manager**（简化 ML 活动权限）+ **Model Cards**（集中记录模型信息）+ **Model Dashboard**（集中查看已部署模型行为）。**Model Monitor**（监控生产模型的数据/预测质量漂移）和 **Clarify**（检测偏差、提供可解释性）虽然也和"负责任地使用模型"相关，但它们属于**模型监控/可解释性工具**，不是 AWS 官方定义的"治理三件套"这一固定组合——选项里如果用 Model Monitor 或 Clarify 替换掉 Model Cards 或 Role Manager 中的任意一个，就不是这个特定问法的正确答案。
...
12. **模型治理工具辨析**：**Model Cards**（标准化模型文档）、**Model Registry 审批工作流**（部署前审批把关）、**Role Manager**（基于角色人设的最小权限 IAM 策略）、**Model Dashboard**（组织级模型集中视图）——四者分工不同，是治理类场景题的高频对比点；官方"治理三件套"特指 **Role Manager + Model Cards + Model Dashboard**，**Model Monitor/Clarify 不算在这个固定组合内**（它们是监控/可解释性工具，不是治理工具）
13. **"实时推理 vs 批量推理"通用二分法**：实时推理=**同步**执行=API 直接调用；批量推理=**异步**执行=批处理框架/按计划触发脚本——方向不能反；此处"异步"是笼统说法，与上方四种部署方式里 SageMaker 具体功能"Asynchronous Inference"不是一回事
