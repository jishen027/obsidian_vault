...
> **🚩 个人高危警示**：这个知识点已经在错题本里**反复出错 3 次**（跨越两个不同题库），说明"看到多模态就选生成模型"的反射还没有被打破。读这一节时，请先**不看答案自己回答一遍**下面这句自测，再往下看解析：
> **自测**：搜索/客服应用需要"看懂"用户发来的文字+图片，选**嵌入模型**还是**生成模型**？──先问自己"这个任务的产出是新内容，还是一个匹配/检索结果"，产出如果不是新内容，答案必须是**嵌入模型**，不管题目怎么包装成"搜索""客服""聊天机器人"等场景。
